---
layout: work
title: About Warding
drafts: |-
  스웨덴 칼마르 테아터갤러리엣(Teatergalleriet) 레지던시 전시.
  스몰란드 트리엔날레 2026(Småland Triennale 2026) 프로그램의 일환으로 개최.

  《About Warding》는 6주간의 레지던시에서 출발한 작업이다. 이 기간 동안 나는 도시 정중앙의 섬 요새에 자리한 칼마르 교도소와, 돌봄과 감금의 기능이 역사적으로 뒤섞여 있던 파리의 오텔디외(Hôtel-Dieu)를 나란히 들여다보았다. '우리'와 '그들'을 가르는 이 경계선을 따라가며, 나는 이 지역의 화강암으로 시선을 돌린다 — 한때 사람들이 손으로 농지에서 걷어내 돌담으로 쌓아올렸다가, 훗날 교외 주택가 정원의 장식석으로 재현된 돌들이다. 콘크리트 잔해를 다뤄온 자신의 기존 조형 언어와 푸케베리(Pukeberg)의 유리 공예 전통 사이를 오가며, 나는 이러한 발견들을 돌, 캐스팅·블로잉 유리, 점토, 실리콘, 석고를 결합한 조각으로 옮겨낸다 — 장소를 불문하고 동일하게 작동하는 보호와 배제의 행정적·건축적 논리를 담아낸 오브제들이다.
types:
  - Exhibition
  - Highlight
year_start: 2026
year_end: 2026
date_start: 2026-06-30
status: Completed
tags:
  - Wall
  - City
  - Map
cover: /images/2026/about-warding/View_about_warding_01_01-01.webp
cover_caption: Exhibition view _About Warding_, Teatergalleriet, Kalmar, Sweden, 2026.
summary: |-
  Residency exhibition at Teatergalleriet, Kalmar, Sweden.

  Presented as part of Småland Triennale 2026.
description: '_About Warding_ developed from a six-week residency during which I examined Kalmar''s prison — an island fortress at the exact center of the city — alongside the Hôtel-Dieu in Paris, another site where care and confinement have historically overlapped. Tracing this line between "us" and "them," I turned to the region''s granite: boulders once cleared from farmland by hand, piled into dry-stone walls, and later reproduced as garden ornaments in suburban housing developments. Working between my established vocabulary in concrete rubble and Pukeberg''s glassblowing tradition, I translated these findings into sculptures combining stone, cast and blown glass, clay, silicone, and plaster — objects that carry the same administrative and architectural logic of protection and exclusion, wherever they are found.'
media:
  - type: image
    src: /images/2026/about-warding/View_about_warding_02_01-03.webp
    caption: '*Intake Form*, cast glass, 34×56×1.5cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_03_01-06-1.webp
    caption: '*Intake Form*, cast glass, 34×56×1.5cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_03_02-07.webp
    caption: '*Untitled*, glass droplet, stone, fly remains, variable dimensions, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_03_04-09.webp
    caption: '*Untitled*, glass droplet, stone, fly remains, variable dimensions, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_03_05-10.webp
    caption: '*Untitled*, glass droplet, stone, fly remains, variable dimensions, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_03_08-13.webp
    caption: '*Untitled*, glass droplet, stone, fly remains, variable dimensions, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_03_10-15.webp
    caption: '*Untitled*, glass droplet, stone, fly remains, variable dimensions, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_03_11-16.webp
    caption: '*Untitled*, glass droplet, stone, fly remains, variable dimensions, 2026.'
  - type: image
    src: /images/about-warding/View_about_warding_04_02-18.webp
    caption: _Cullet Fountain_, glass cullet, water, 58×65×33cm, 2026.
  - type: image
    src: /images/2026/about-warding/View_about_warding_04_03-19.webp
    caption: _Cullet Fountain_, glass cullet, water, 58×65×33cm, 2026.
  - type: image
    src: /images/about-warding/View_about_warding_05_02-23.webp
    caption: _Vårda Kalmar_, cast glass , 23×22.5×1.5cm, 2026.
  - type: image
    src: /images/2026/about-warding/View_about_warding_05_01-22.webp
    caption: Exhibition view _About Warding_, Teatergalleriet, Kalmar, Sweden, 2026.
  - type: image
    src: /images/about-warding/View_about_warding_06_01-26.webp
    caption: _CCTV_, glass cullet, plaster, fly remains, ⌀12×8.5cm, 2026.
  - type: image
    src: /images/2026/about-warding/View_about_warding_07_02-28.webp
    caption: Exhibition view *About Warding*, Teatergalleriet, Kalmar, Sweden, 2026.
  - type: image
    src: /images/2026/about-warding/View_about_warding_08_01-29.webp
    caption: '*Crunch*, stone, plaster, 50×16×1cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_08_04-32.webp
    caption: '*Crunch*, stone, plaster, 50×16×1cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_08_05-33.webp
    caption: '*Crunch*, stone, plaster, 50×16×1cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_09_01-35.webp
    caption: '*Walking With*, plaster, 21×29.6×1cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_09_02-36.webp
    caption: '*Walking With*, plaster, 21×29.6×1cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_10_01-37.webp
    caption: '*Kalmar Plan*, pigmented plaster, 38×26.5×1cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_10_02-38.webp
    caption: '*Kalmar Plan*, pigmented plaster, 38×26.5×1cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_11_01-39.webp
    caption: Exhibition view *About Warding*, Teatergalleriet, Kalmar, Sweden, 2026.
  - type: image
    src: /images/2026/about-warding/View_about_warding_11_02-40.webp
    caption: '*CCTV*, glass cullet, plaster, fly remains, ⌀12×8.5cm, 2026.'
  - type: image
    src: /images/2026/about-warding/View_about_warding_11_03-41.webp
    caption: '*CCTV*, glass cullet, plaster, fly remains, ⌀12×8.5cm, 2026.'
hide_cover_if_video_top: false
tables: ''
linked_works_title: Linked works
linked_works:
  - label: '*About Warding* (text) (2026)'
    path: /work/2026/text_about-warding.html
critique_title: Related texts
related_texts:
  - id: won-jy-kalmar
---
