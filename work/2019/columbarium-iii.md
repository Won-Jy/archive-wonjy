...
drafts: |-
  Pierrette Gaudiat가 기획한 Château d'Aubais에서의 단체전 "Trop bête ! l'homme et sa nature" (2019)에 초대되어서 나의 비둘기 무덤들을 전시할수있게됨.

  이 전시를 일환으로, 집에서 정리할때 쌓았던 비둘기 무덤 상자들을 일렬로만 높게 쌓아 탑처럼 만들고 이를 "Columbarium"이라고 이름붙였다. 이는 내가 앞으로 하게될 비둘기 유해에 관련된 모든 행위들에 지칭하게된다. 상자들을 쌓을때, 밑에서부터 매장한지 오래된 시간 순서대로 쌓았고 그래서 상단에는 비교적 최근에 매장된 무덤들이 올라가게되었다. 전시가 열릴때에는 맨위의 상자에는 그안에 태어난 수백의 구더기들이 파리가 되어있었다.

  그때 마지막으로 사용했던 플라스틱 상자에는 상단부분에 작게 숨구멍이 나있었는데 (파리가 그를 통해 나가기엔 너무 작은 구멍이였다.), 그들은 그 구멍에 몰려있거나, 박스 뚜껑 조인트 공간사이를 파고들다가 거기안에 끼어있게되는게 일수였다.
...
media:
  - type: image
    ...
    caption: Flies born inside the box, gathered near a hole in its side.
  - type: image
    src: /images/columbarium-iii/Columbarium_III_03.webp
    caption: Flies that died inside the box.
